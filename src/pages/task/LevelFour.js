...
export default class LevelOne extends Component {
    ...
    handleShowAnswer=async ()=>{
        ...
        if(showAnswer)
        {
            if((!submitAnswer[3])&&(!showAnswer[3]))
            {
                showAnswer[3]=true;
                const userRef=firestore.doc(`users/${id}`);       
            ...
            <Jumbotron className='task-page task-content-container'>
               ...
               {/* <Row><h3>{question}</h3></Row> */}
                ...
                />
                    ...
                    {
                        showAnswer?(<Button variant="outline-primary" style={{margin:'0px 5px'}}><Link to='/level3'> Previous Level</Link></Button>):null
                    }
                    {
                     showAnswer?(showAnswer[3]?<Button variant="outline-primary" style={{margin:'0px 5px'}}><Link to='/level5'> Next Level</Link></Button>:(submitAnswer?(submitAnswer[3]?<Button variant="outline-primary" style={{margin:'0px 5px'}}><Link to='/level5'> Next Level</Link></Button>:null):null)):null
                    }
