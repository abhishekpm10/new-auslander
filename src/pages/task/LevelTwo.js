...
export default class LevelOne extends Component {
    ...
    handleHint=async ()=>{
            ...
            if(hint)
            {
                if((!submitAnswer[1])&&(!hint[1])&&(!showAnswer[1]))
                {
                    hint[1]=new Date();
    ...
    handleShowAnswer=async ()=>{
        ...
        {
            if((!submitAnswer[1])&&(!showAnswer[1])&&(tmpVal<=4200))
            {
                showAnswer[1]=new Date();
                const userRef=firestore.doc(`users/${id}`);       
    ...
    handleUserAnswer =async ()=>{
        ...
        if(this.props.correctAnswer===this.state.userAnswer)
        {
                    ...
                    if(showAnswer)
                    {
                        if(!showAnswer[1]&&!submitAnswer[1])
                        {
                            submitAnswer[1]=new Date();
        ...
        return (
            <Jumbotron className='two-task-page'>
               ...
               <div className="outside-book">
                    {
                        hint?(hint[1]?<Row><h3>The Hint Is:{dataString}</h3></Row>:null):null
                    }
                    {
                        showAnswer?(showAnswer[1]?<Row><h3>The Answer Is:{correctAnswer}</h3></Row>:null):null
                    }
                    ...
                    {
                        showAnswer?(<Button variant="outline-primary" style={{margin:'0px 5px'}}><Link to='/theroyalmurder'> Previous Level</Link></Button>):null
                    }
                    {
                     showAnswer?(showAnswer[1]?<Button variant="success" style={{margin:'0px 5px'}}><Link to='/crosswood'> Next Level</Link></Button>:(submitAnswer?(submitAnswer[1]?<Button variant="success" style={{margin:'0px 5px'}}><Link to='/crosswood'> Next Level</Link></Button>:null):null)):null
                    }
                </div>
